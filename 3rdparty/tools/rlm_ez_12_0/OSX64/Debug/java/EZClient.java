 ...
 *      1/14
 *
 *      $Id: EZClient.java,v 1.2 2016/01/13 02:16:56 matt Exp $
 */
...
public class EZClient {
	private static final String defProduct = "test1";
	...
	private static void doDemo(String product) {
		...
		boolean yes = yesNo();
		if (yes) {
			int stat = RlmEZ.deleteDemo(product, version);
			if (stat == 0) {
			    System.out.println("Existing demo deleted");
			    stat = RlmEZ.createDemo(product, version, 10);
			    if (stat == 0)
				System.out.println(
	...
	public static void main(String argv[]) {
		...
		if (argv.length > 0)
			...
			product = defProduct;
		int days = RlmEZ.checkout(product, version);
		stat = days;
		if (days >= 0) {
