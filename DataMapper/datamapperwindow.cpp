...
bool DataMapperWindow::ValidateNewDatasource(const QString& datasource) {
	...
	if (SynGlyphX::DatabaseServices::IsSQLiteDB(datasource)) {
		return true;
	}
...
void DataMapperWindow::AddDataSources() {
	...
	for (const QString& datasource : dataSources) {
		...
		SynGlyphX::DatabaseServices::AddDatabaseConnection(newDatasource, newDBID);
		...
		try {
			...
			if (newDatasource.CanDatasourceHaveMultipleTables()) {
				...
				QStringList qtables = SynGlyphX::DatabaseServices::GetListOfTablesWithoutAutogeneratedTables(db);
