 ...
 ** MODIFIED: 2014.05.13  ABS  restored corners input
 ** MODIFIED: 2014.05.23  ABS  fixed off-by-one bug producing glyphs at 0,0,0
 ** MODIFIED: 2014.05.25  ABS  ignores blank lines, better EOF detection
 ** MODIFIED: 2014.05.28  ABS  tracking down bug Ray found
 **
 **/
...
char** argv;
{
    ...
    if (Debug != FALSE) {
        ...
        fprintf(stderr, "%s: DEBUG: CornersFile = %s\n",
          ProgName, CornersFile);
        fprintf(stderr, "%s: DEBUG: CornersTemplateFile = %s\n",
          ProgName, CornersTemplateFile);
        ...
        fprintf(stderr, "%s: DEBUG: SW_lat_in = %f\n", ProgName, SW_lat_in);
        fprintf(stderr, "%s: DEBUG: SW_lon_in = %f\n", ProgName, SW_lon_in);
        fprintf(stderr, "%s: DEBUG: NE_lat_in = %f\n", ProgName, NE_lat_in);
        fprintf(stderr, "%s: DEBUG: NE_lon_in = %f\n", ProgName, NE_lon_in);
        }
    ...
    /* compute corners or set from flags */
    ...
    if (PrintCorners) {
        ...
        if (x_fp == NULL)  {
            fprintf(stderr, "%s: ERROR: fopen of %s failed, error: %s\n",
              ProgName, TextFile, strerror(errno));
            exit(0);
            }
        ...
        sprintf(sw_lat_str, "%.8lf", SW_lat);
        sprintf(sw_lon_str, "%.8lf", SW_lon);
        sprintf(ne_lat_str, "%.8lf", NE_lat);
        sprintf(ne_lon_str, "%.8lf", NE_lon);

        template_lines = lines_in_file(CornersTemplateFile);

        c_fp = fopen(CornersTemplateFile, "r");
        if (c_fp == NULL)  {
            fprintf(stderr, "%s: ERROR: fopen of %s failed, error: %s\n",
              ProgName, CornersTemplateFile, strerror(errno));
            exit(0);
            }

        k_fp = fopen(CornersFile, "w");
        if (k_fp == NULL)  {
            fprintf(stderr, "%s: ERROR: fopen of %s failed, error: %s\n",
              ProgName, CornersFile, strerror(errno));
            exit(0);
            }
    ...
    for (row = 0; row < ActualNumDataLines; row++) {
        ...
        v = 0;
        for (g = old_id; g < id; g++) {
            fprintf(t_fp, "%d,%d,0,\"%s - var %d\",\"unused\"\r\n",
              i++, g, Tags[row + 1], v++);
