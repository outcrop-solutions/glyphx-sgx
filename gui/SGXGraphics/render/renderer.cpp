	...
	namespace render
	{
		...
		void renderer::render_batches(hal::context* context, camera* _camera, batch_set& b)
		{
			for (auto& bl : b)
			{
				...
				for (auto& batch : bl.second)
				{
					const auto& parts = batch.m->get_parts();
					auto transform = batch.transform * batch.m->get_transform();
					for (auto part : parts)
					{
						context->set_constant(batch.e, "material", "tint_color", batch.tint_color);
						context->set_constant(batch.e, "instance_data", "world", transform * part->get_transform());
