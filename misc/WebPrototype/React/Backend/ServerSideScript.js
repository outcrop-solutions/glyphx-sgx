...
function authenticateUser(data){
	...
	if(UserAccountsResult && UserAccountsResult != "" && UserAccountsResult != null && (hash == UserAccountsResult[0].Password || data.password == UserAccountsResult[0].Password))
	{
		...
		//delete the from the return object
		...
		var securityGroupQuery = "SELECT * FROM SecurityGroups LEFT JOIN AvailableGroups ON AvailableGroups.Group = SecurityGroups.ID WHERE AvailableGroups.UserID = '"+ UserAccountsResult[0].ID +"'";
...
function saveViewInRecentViews(viewObj,userId){
	...
	if(fileContent && util.isArray(fileContent)){
		if(fileContent.length > MAX_RECENT_VIEWS_PER_USER){
			fileContent.shift();//remove the 1st element. We follow a first in first out procedure.
		}
...
function checkUser(req, res, next) {
  if ( req.path == '/login' || req.path.includes('/customerImg') || req.path.includes('/viz/') || req.path.includes('test')) return next();
...
app.get('/login', async (req, res, next) => {
	if (!req.query.username || !req.query.password) return res.sendStatus(400);
...
app.post('/manageAnnouncements', async (req, res, next) => {
	if (!req.body && !req.body.type) return res.sendStatus(400);
...
app.post('/saveUserSettings', async (req, res, next) => {
	if (!req.body && !req.body.userSettings) return res.sendStatus(400);
...
app.post('/checkFrontEndFilterQuery', async (req, res, next) => {
	if (!req.body && !req.body.frontEndFilters) return res.sendStatus(400);
	...
	for(outerIndex = 0; outerIndex < data.length; outerIndex++){
		...
		dataItem.shift(); // removes the 1st element that is the name.
...
app.post('/saveView', async (req, res, next) => {
	...
	var insertQuery = "INSERT INTO UserSavedVisualizations "
						+ "(Name,OriginalVizName,UserID,QueryString,InstitutionId,frontEndFilterString) VALUES (?)";
	var insertQueryValues = "'"+savedVizName+"','"+ originalVizName+ "','"+sess.user.userInformation.ID+ "','" + filterQuery + "','"+ sess.user.userInformation.Institution+"','"+frontEndFilterString+"'";
	insertQuery = insertQuery.replace('?',insertQueryValues);
		
	//check wether the same named viz already exists. If it does then just update the query string created.
	try{
		if(vizId != null){
...
app.post('/loadVisualization', async (req, res, next) => {
	if (!req.body || (req.body && !req.body.query)) return res.sendStatus(400);
...
app.get('/frontEndFilterData/:sdtFile', async (req, res, next) => {
	if (!req.params.sdtFile) return res.sendStatus(400);
	
	res.setHeader('Cache-Control', 'public, max-age=' + (7200)); //Set to 2 hours.
