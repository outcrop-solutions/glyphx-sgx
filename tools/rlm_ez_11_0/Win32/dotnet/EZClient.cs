...
namespace Reprise
{
    class RLMEZclient
    {
	[DllImport("rlmez1102.dll", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
	public static extern int rlmez_checkout(String version, ref int days);

	[DllImport("rlmez1102.dll", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
	public static extern int rlmez_demo(String version, int days);

	[DllImport("rlmez1102.dll", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
	public static extern int rlmez_demox(String version);
	...
	private static void doDemo() {
		...
		bool yes = yesNo();
		if (yes) {
			int stat = rlmez_demox(version);
			if (stat == 0) {
			    Console.WriteLine("Existing demo deleted");
			    stat = rlmez_demo(version, 10);
			    if (stat == 0)
	...
	static void Main() {
		int stat, days = 0;
		bool yes;
		stat = rlmez_checkout(version, ref days);
		if (days >= 0) {
